...
# train test split
train, test = train_test_split(series, test_size=0.4, shuffle=False)

train_shifted = train.shift(periods=365)
train_diff = train - train_shifted

test_shifted = test.shift(periods=365)
...
y_original = y_diff + test_shifted.values[(365 + 3):]

np.abs(y_original - test.values[(365 + 3):]).mean()
